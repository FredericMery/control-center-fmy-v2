    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-20 text-blue-950">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-600">
            {t('settings.memoryZone.subtitle')}
          </p>
        </div>
        <Link
          href="/dashboard/settings"
          className="rounded-lg bg-gray-200 px-3 py-2 text-sm text-gray-800"
        >
      ...
      {toastMessage && (
        <div className="fixed left-4 right-4 top-4 z-50 sm:left-auto sm:right-6 sm:top-6 sm:w-[380px]">
          <div
            role="status"
            className="rounded-lg border border-green-300 bg-green-50 px-3 py-2 text-sm text-green-800 shadow-lg"
          >
            {toastMessage}
          </div>
        </div>
      ...
      {selectedSectionId && (
        <div className="rounded-2xl bg-white p-6 shadow-sm space-y-4">
          ...
          {(saving || progress === 100) && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-600">
                {t('settings.memoryZone.progressLabel', { progress })}
              </p>
            </div>
          )}
          ...
          <p className="text-xs text-gray-500">
            {loadingItems
              ? t('settings.memoryZone.updatingNow')
              : t('settings.memoryZone.cardsCount', { count: sectionItemsCount })}
          </p>
